...
export class RequestContext {
   ...
   */
  static create(em: EntityManager | EntityManager[], query: any, next: (...args: any[]) => void): void {
    const ctx = this.createContext(em, query);
  ...
  /**
   * Returns current Request Query (if available).
   */
  static getQuery(name = 'query'): any | undefined {
    const context = RequestContext.currentRequestContext();
    return context ? context.map.get(name) : undefined;
  }

  private static createContext(em: EntityManager | EntityManager[], query?: any): RequestContext {
    const forks = new Map<string, EntityManager>();

    if (query) {
      forks.set('query', query);
    }
