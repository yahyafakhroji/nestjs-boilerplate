  ...
  public static init(data: any, rules: Rules, customMessages?: ErrorMessages): Validate.Validator<any> {
    Validate.register(
      ...
      function (value) {
        ...
        return values.every((item) => item && item.length === 26);
      },
    ...
    Validate.register(
      ...
      // tslint:disable-next-line:only-arrow-functions
      function (value) {
        return /^(^\+62\s?|^0)(\d{3,4}-?){2}\d{3,4}$/g.test(String(value).replace(/[^0-9+]/g, ''));
      },
    ...
    Validate.register(
      ...
      function (value) {
        const values: any[] = Array.isArray(value) ? value : [value];
        const unique = values.filter((item, i, ar) => ar.indexOf(item) === i);
    ...
    Validate.register(
      ...
      function (value) {
        const values: any[] = Array.isArray(value) ? value : [value];
        return values.every((item) => /^[a-zA-Z0-9_\- ]+$/.test(item));
      },
    ...
    Validate.register(
      ...
      // tslint:disable-next-line:only-arrow-functions
      function (value) {
        return /^([-+]?)([\d]{1,2})(((\.)(\d+)(,)))(\s*)(([-+]?)([\d]{1,3})((\.)(\d+))?)$/g.test(String(value));
      },
